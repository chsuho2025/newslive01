module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // 3차: HyperCLOVA X — 필터링 + 선별 + 제목 다듬기 + 요약
    var newsListText = cleaned.map(function(n) {
      return n.no + '. [제목] ' + n.title + '\n   [설명] ' + (n.description || '').slice(0, 80);
    }).join('\n');

    var systemPrompt = [
      '당신은 뉴스 편집 AI입니다. 아래 규칙을 반드시 따르세요.',
      '',
      '[필터링 규칙]',
      '- 광고성, 낚시성, 어뷰징 기사 제외',
      '- 연예/스포츠/날씨 제외 (단, 사회적 파장이 큰 경우 포함)',
      '- 같은 사건을 다룬 기사가 여러 개면 1개만 선택',
      '',
      '[선택 기준]',
      '- 정치, 경제, 사회, 국제 이슈 우선',
      '- 최대한 다양한 주제로 10개 선별',
      ...
      '- 형식: [{"no":원본번호,"title":"다듬은 제목(20자 이내)","summary":"핵심 요약(30자 이내)"},...]',
      '- title: 원본 제목을 간결하고 명확하게 다듬기. 특수문자/대괄호 제거.',
      '- summary: 설명을 바탕으로 핵심만 한 문장으로 요약.',
      '- 정확히 10개 출력.'
    ].join('\n');

    var userPrompt = '다음 뉴스 20개에서 중요한 10개를 골라 가공해줘:\n\n' + newsListText;
