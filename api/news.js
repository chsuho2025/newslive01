module.exports = async function handler(req, res) {
  ...
  try {
    ...
    var clovaRes = await fetch(
      'https://clovastudio.stream.ntruss.com/serviceapp/v3/chat-completions',
      {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ model: 'HCX-DASH-002',
          messages: [
    ...
    console.log('CLOVA 응답:', JSON.stringify(clovaData));
    var rawText = (clovaData && clovaData.result && clovaData.result.message && clovaData.result.message.content) ? clovaData.result.message.content : '';
